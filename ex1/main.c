...
*******************************************************************************/
    ...
    while(1)
    {
        /* Here each LEDs blinks twice in a period of 0.5 s, that means total 4 times blinking in 1 s.*/
        /* LED1 red on for half period with 2 times blinking. */
        P4OUT |= BIT6;                  // Turn LED1 on.
        P1OUT &= ~BIT0;                 // Turn LED2 off.
        __delay_cycles(125000);         // Delay for 0.125 s.
        ...
        uartTx("Green LED\'s turn \r\n ");  // Transmit a message every half period.

        /* LED2 green on for half period with 2 times blinking. */
        P4OUT &= ~BIT6;                 // Turn LED1 off.
